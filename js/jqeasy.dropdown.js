...
$(document).ready(function() {
	$('.btnsignup').click(function(e) {
		e.preventDefault();
		$("#frmsignup").toggle('fast',function() {
				$('#username').focus();
			});
		$(this).toggleClass("btnsignupon");
		$('#msg').empty();
	});
	...
	/*$(document).mouseup(function(e) {
		if($(e.target).parents('#frmsignup').length==0) {
			$('.btnsignup').removeClass('btnsigninupon');
			$('#frmsignup').hide('fast');
		};
	});
	...
	$('#signup').ajaxForm({
		...
		success: function(data) {
			if (data=='OK') {
				$('#frmsignup').text('Signed in!');
				$('#frmsignup').delay(800).fadeOut(400);
				$('#signbtn').html('<a href="logout.asp" class="btnsignout">Sign Out</a>');
			} else {
				$('#msg').html(data);
				$('#username').focus();
