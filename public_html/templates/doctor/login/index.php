...
if(isset($_POST['btnSignIn'])) {
		...
		$query = "SELECT * FROM ".Doctor." WHERE email = '$user_email' and password = '$password' and isactive=1 and user_type='Admin'";
		//echo $query;
		$rows = $db->select($query);
		//Alert ($db->row_count);
		if($db->row_count > 0) {
			while($row = $db->get_row($rows, 'MYSQL_ASSOC')) {
				$_SESSION['UserDocID'] = $row['doctor_id'];
				...
				Redirect($strHostName."/page_doctor.php?dir=doctor/profile");
			}			
		} 
		else {
			AlertandRedirect("Login failed. Invalid Username or Password.", "page_doctor.php?dir=doctor/login");
		}
	}
  ...
  <div class="top_ou">
    <div class="top_in">
      <div class="top" style="padding-top:100px">
        <h1 style="font-weight: bold; font-size: 35px;"><span class="f_green"> Doctor</span> <span class="f_blue">Login</span></h1>
        <div id="loginform">
          ...
          <div class="dvFloat formpadding">
            ...
            <div class="formcontrol">
              <a href="./page_doctor.php?dir=doctor/forgot_password">Forgot Password</a>
            </div>
          </div>
