        ...
        <div class="cal_12" style="text-align: justify; padding:45px 0;">
          ...
          <div class="DvFloat" style="border: solid 0px #0066CC;">
            <ul id="countrytabs" class="shadetabs">
              <li><a href="<?php echo $strHostName;?>/clerk_admin.php" rel="country1" class="selected" style="display:none;">Patients Not Accepted</a></li>
            ...
            <div style="border: 0px solid gray; width: 965px; float: left; margin-bottom: 1em; padding: 10px 0px; background-color: #FFFFFF; color: #666666;">
              <div class="dvFloat"> 
              	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                      ...
                      <?php  
						$query = "SELECT * FROM  ".Compose." where isactive=1 and isdeleted=0 and user_id not in (select patient_id from tbl_patients)";
						//echo $query;
							...
							if($result != "") {	
								...
								if($rowcount > 0) {
									while($row = mysql_fetch_assoc($result)) {
										extract($row);
										$name=GetValue("select name as col from tbl_users where user_id=".$row['user_id'], "col");
										$dob=GetValue("select dob as col from tbl_users where user_id=".$row['user_id'], "col");
										$gender=GetValue("select gender as col from tbl_users where user_id=".$row['user_id'], "col");
										$mobile=GetValue("select mobile as col from tbl_users where user_id=".$row['user_id'], "col");
										$created_date=$row['created_date'];
										
										$seconds = strtotime(date('Y-m-d h:i:s')) - strtotime($created_date);
                       ...
                       <tr>
                              ...
                              <td class="tdborder" style="padding:10px 20px;"><?php echo date('d-M-Y h:i:s',strtotime($created_date));?></td>
