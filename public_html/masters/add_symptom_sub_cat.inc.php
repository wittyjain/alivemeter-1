...
<?php
$id=""; $pcatid=""; $sub_cat_name =""; $isdeleted=""; $condition=""; $maincatid=""; $qs="";$isactive="1"; $createdate=date('d-m-Y h:i:s'); 
...
?>
	...
	if(isset($_POST['btnSubmit'])) {
		$sub_cat_name = trim(str_replace("'", "''", $_POST['txtCatName']));
		...
		if($id == "" || $id==0) {
			$id = $db->insert_array(Symptom_Sub_Cat, $data);
					
			if (!$id) { 
				$db->print_last_error(false);
			} else {
				AlertandRedirect("New symptom sub category is added successfully", "page.php?dir=master/symptom_sub_cat/add");
			}
		} else {
			$rows = $db->update_array(Symptom_Sub_Cat, $data, "sub_cat_id = $id");
			if (!$rows) {
				$db->print_last_error(false);
			} else {
				if ($rows > 0) {
					AlertandRedirect('Symptom sub category is edited successfully',  "page.php?dir=master/symptom_sub_cat/view");
				}
			}
		}
	} 
	...
	{
		$query = "SELECT * FROM ".Symptom_Sub_Cat." WHERE sub_cat_id = $id";
		//echo $query;
                 ...
                 <div class="dvFloat formpadding1" style="padding-top:20px">
                    <div class="formlabel1">
                      <label class="formlabel1" style="padding-left:0px; padding-bottom:15px;">
                      	 <h2 class="Tab_Title">Add Symptom Sub Category</h2>
                     </label>
                    </div>
                  ...
                  <div class="dvFloat formpadding1">
                    <div class="formlabel1">
                      <label class="formlabel1">Sub Category Name<span class="redtxt">*</span> </label>
                    </div>
                    <div class="formcontrol2">
                      <input type="text" name="txtCatName" id="txtCatName" value="<?php echo $sub_cat_name	;?>" />
