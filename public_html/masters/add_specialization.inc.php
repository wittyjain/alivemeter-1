...
function Validation() {
	...
	if(document.getElementById("txtSpecialization").value == "") {
		alert("Please Enter Specialization Title...");
...
<?php
$id=""; $pcatid=""; $specialization_name =""; $isdeleted=""; $condition=""; $maincatid=""; $qs="";$isactive="1"; $createdate=date('d-m-Y h:i:s'); 
...
?>
	...
	if(isset($_POST['btnSubmit'])) {
		$specialization_name = trim(str_replace("'", "''", $_POST['txtSpecialization']));
		...
		if($id == "" || $id==0) {
			$id = $db->insert_array(Specialization, $data);
					
			if (!$id) { 
				$db->print_last_error(false);
			} else {
				AlertandRedirect("New Specialization is added successfully", "page.php?dir=master/specialization/add");
			}
		} else {
			$rows = $db->update_array(Specialization, $data, "Specialization_id = $id");
			if (!$rows) {
				$db->print_last_error(false);
			} else {
				if ($rows > 0) {
					AlertandRedirect('Specialization is edited successfully',  "page.php?dir=master/specialization/view");
				}
			}
		}
	} 
                 ...
                 <div class="dvFloat formpadding1" style="padding-top:20px">
                    <div class="formlabel1">
                      <label class="formlabel1" style="padding-left:0px; padding-bottom:15px;">
                      	 <h2 class="Tab_Title">Add Specialization</h2>
                     </label>
                    </div>
                  ...
                  <div class="dvFloat formpadding1">
                    ...
                    <div class="formcontrol2">
                      <input type="text" name="txtSpecialization" id="txtSpecialization" value="<?php echo $specialization_name;?>" />
                       <input id="txt_SpecializationID" name="txt_SpecializationID" value="" type="hidden"/>
                    </div>
                  </div>
