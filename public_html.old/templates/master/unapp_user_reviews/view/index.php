...
function UnApproveReviews()
{
	
	if(confirm ("Are you sure want to approve selected comments?"))
	{
		...
		xmlhttp.open("GET","<?php echo $strHostName; ?>/includes/approve_review.inc.php?cmtid="+cmtid+"&type=Approve", true);
		xmlhttp.send();
	}
	else
	{
...
function Delete_Blog_By_Id()
{
	...
	if(confirm ("Are you sure want to delete selected comments?"))
	{
		...
		xmlhttp.open("GET","<?php echo $strHostName; ?>/includes/delete_review.inc.php?cmtid="+cmtid+"&type=Approve", true);
		xmlhttp.send();
	}
	else
	{
                 ...
                 <div class="dvFloat formpadding1" style="padding-top:20px">
                    <div class="formcontrol2" style="padding-left:0px; padding-bottom:10px; ">
                      <label class="formcontrol2">
                      	 <h2 class="Tab_Title">View User Reviews</h2>
                     </label>
                    </div>
                   
                    <div class="formcontrol2" style="padding-left:0px; padding-bottom:10px; ">
                      <label class="formcontrol2">
                      	 <div style="float:left; width:200px; margin-right:5px;"><a href="<?php echo $strHostName;?>/page.php?dir=master/reviews/view" class="button2" style="width:180px;">Approved User Reviews</a></div>
                         <div style="float:left; width:300px;"><a href="<?php echo $strHostName;?>/page.php?dir=master/unapp_user_reviews/view" class="button2" style="width:250px;  background-color:#009999;">Unapproved User Reviews</a></div>
                     </label>
                    </div>
                  		...
                  		<table cellpadding="0" cellspacing="0"  style="width:100%;float:left;" >
                              <tr>
                                <td class="tbl_head" style="width:20%; text-align:center;">User Name </td>
                                <td class="tbl_head" style="width:20%; text-align:center;">User Reviews</td>
                                <td class="tbl_head" style="width:20%; text-align:center;">Date</td>
                                <td class="tbl_head" style="width:20%; text-align:center;">Reviews for</td>
                                <td class="tbl_head" style="width:10%; text-align:left;">Approval</td>
                                <td class="tbl_head" style="text-align:center; padding-left:0px; width:10%;">Delete</td>
                              </tr>
                               <?php $retrive_Array=$get_retrive->Get_Un_User_Reviews()  ?>
                              <?php  
							  $i=0;
							  $i=$i+1;
							  while($get_array = mysql_fetch_array( $retrive_Array )){?>
                              <tr id="tr_post_comment_<?php echo $get_array['user_review_id']*121?>">
                                ...
                                <td class="tdborder" style="padding-left:20px;"><?php echo $get_array['comment']?> </td>
                                 ...
                                 <td class="tdborder" style="padding-left:20px;">
                                 	<?php
										$name=GetValue("select doctor_name as col from tbl_doctor where doctor_id=".$get_array['common_id']." and type='".$get_array['common_type']."'", "col");
									echo $name ;
									?>
                                    (<?php echo $get_array['common_type'];?>)
                                 </td>
                                 ...
                                 <?php /*?>	<input type="checkbox" name="chkApproval" id="chkApproval<?php echo $i?>" <?php if($get_array['approved']=="1") { echo "checked";}?> /><?php */?>
                                    ...
                                    <input type="checkbox" name="chkApproval" id="chkApproval<?php echo $i?>"/>
                                      ...
                                      <input type="hidden" id="txtCommentid<?php echo $i?>" name="txtCommentid<?php echo $i?>" value="<?php echo $get_array['user_review_id']?>" style="width:20px;"/>
                                 </td>
                              ...
                              <input type="hidden" id="txtCmtCount" name="txtCmtCount" value="<?php echo $i?>"/>
                                <div class="formcontrol2" style="width:400px; float:left; padding:15px 0;">   
                                  Total UnApproved Reviews : 
                                  <?php 
								  	$total_app_cmt=GetValue("select Count(*) as col from ".User_Reviews."  where approved=0", "col");
									echo $total_app_cmt;
								  ?>
                                </div>
                                  ...
                                  <div class="formcontrol2" style="width:300px; float:right; padding:15px 0;">   
                                    <a  onclick="javascript:Delete_Blog_By_Id()"  style="float:right; width:80px; font-size:14px; text-align:center;padding:5px 0px;" class="button">Delete</a>
                                    <a name="btnSubmit" id="btnSubmit" class="button" onclick="javascript:UnApproveReviews();" style="float:right; width:80px; font-size:14px; text-align:center; padding:5px 0;  margin-right:10px; ">Submit </a>
